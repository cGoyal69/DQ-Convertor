function jsonToMongo(jsonObject) {
    ...
    function generateAggregateQuery(intermediate) {
      const { collection, pipeline, limit, sort } = intermediate;
      ...
      let query = `db.${collection}.aggregate(${JSON.stringify(processedPipeline)})`;
  
      if (limit) query += `.limit(${limit})`;
      if (sort) query += `.sort(${JSON.stringify(convertSort(sort))})`;
    ...
    return testIntermediateToMongo(sampleIntermediateJson)
  }
  ...
  console.log(jsonToMongo(sampleIntermediateJson));