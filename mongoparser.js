function parseMongoQuery(queryString) {
  ...
  const mongoPattern = /db\.(\w+)\.(find|insert|update|delete|aggregate)\(([\s\S]*?)\)(\.\w+\([\s\S]*?\))*/;
  ...
  const chainedPattern = /\.(\w+)\(([\s\S]*?)\)/g;
  let chainedMatch;
  while ((chainedMatch = chainedPattern.exec(chainedOperations)) !== null) {
    const method = chainedMatch[1];
    const args = eval(`(${chainedMatch[2]})`); // Safely parse the arguments of the chained method
